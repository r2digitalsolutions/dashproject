			...
			<div>
				<a
					><img
						...
						alt="logo"
						class="mb-12 inline-block w-44 lg:w-52"
					/>
				</a>
				<h2 class="text-2xl font-extrabold text-white lg:text-4xl lg:leading-[50px]">
				...
				<p class="mt-6 text-sm text-white">
					Don't have an account <a class="ml-1 font-semibold text-white underline">Register here</a>
				</p>
			</div>

			<form
				class="w-full max-w-md space-y-6 rounded-xl bg-white px-6 py-8 md:ml-auto"
				method="POST"
				action="?/login"
				use:enhance
			>
				<h3 class="mb-6 text-3xl font-extrabold">Sign in</h3>
				{#if form?.message}
					<div class="rounded-md bg-red-100 p-4 text-sm text-red-500">{form.message}</div>
				{/if}
				<div class="space-y-3">
					<div>
						<input
							...
							autocomplete="email"
							required
							class="w-full rounded-md bg-gray-100 px-4 py-3.5 text-sm outline-gray-800 focus:bg-transparent"
